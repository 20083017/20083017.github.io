...
layout:     post
title:      C/C++ 编译相关整理
subtitle:   编译流程 / 静态动态库 / GCC 优化选项 / FDO / AutoFDO / LTO / BOLT / LLVM 笔记汇总
date:       2024-09-06
author:     BY
...
>
>- **ELF**：Executable and Linkable Format，Linux 下可执行文件 / `.so` / `.o` 的标准格式。
>- **TU / 编译单元**：Translation Unit，一个 `.c` / `.cpp` 经过预处理后送给编译器的整体输入。
>- **IR**：Intermediate Representation，编译器中间表示。GCC 的叫 GIMPLE，LLVM 的叫 LLVM IR。
